...
| Setting |
|---|
| `Spreading Factor: AT+PARAMETER=8,7,1,8          (see include/behaviour_config.inc)` |
| `Network ID: AT+NETWORKID=18                     (see include/behaviour_config.inc)` |
| `Mode: AT+MODE=0                                 (see include/behaviour_config.inc)` |
| `Output Power: AT+CRFOP=20                       (see include/behaviour_config.inc)` |
| `Receiver Address: AT+ADDRESS=7775               (see include/behaviour_config.inc)` |
| `Sender Address: AT+ADDRESS=7776                 (see include/behaviour_config.inc)` |
| `Frequencies: Frequency hopping with 1 of the 20 tables containing 50 shuffled frequencies (908 MHz to 920.25 MHz). When sorted, distance between adjacent frequencies is 250 kHz (see include/frequency_variations.inc)` |
